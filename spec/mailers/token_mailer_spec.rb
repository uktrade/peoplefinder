  ...
  describe '.new_token_email' do
    ...
    it 'includes the app guidance' do
      %w(plain html).each do |part_type|
        ...
        expect(get_message_part(mail, part_type)).to have_text('http://test.local') if part_type == 'plain'
        expect(get_message_part(mail, part_type)).to have_link('DIT Intranet', href: 'http://test.local') if part_type == 'html'
      end
    end
